export default ({ strapi }) => ({
  ...
  async processInvitationReward(ctx) {
    try {
      ...
      const result = await strapi.service('api::investment-service.investment-service').processInvitationRewardV2(order);
  ...
  async getInvestmentStats(ctx) {
    try {
      ...
      // Call investment service to get investment stats
