...
async function main() {
  ...
  try {
    ...
    // 5. 检查邀请关系
    console.log('\n5️⃣ 检查邀请关系...');
    const teamResult = await getTeam(inviterToken);
    if (teamResult.success) {
      console.log('✅ 邀请关系建立成功');
      console.log('团队成员:', teamResult.members.map(m => m.username));
    } else {
      console.error('❌ 邀请关系检查失败:', teamResult.message);
    }
    ...
    const orderId = investResult.order.id;
    console.log('✅ 投资成功, 订单ID:', orderId);
    
    // 7. 检查订单状态
    console.log('\n7️⃣ 检查订单状态...');
    const orderResult = await getOrderDetail(inviteeToken, orderId);
    ...
    // 8. 模拟订单到期（赎回）
    console.log('\n8️⃣ 模拟订单赎回...');
    const redeemResult = await redeemOrder(inviteeToken, orderId);
    if (!redeemResult.success) {
      console.error('❌ 赎回失败:', redeemResult.message);
      return;
    }
    console.log('✅ 赎回成功');
...
async function getTeam(token) {
  try {
    const response = await axios.get(`${BASE_URL}/api/auth/my-team`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
async function redeemOrder(token, orderId) {
  try {
    const response = await axios.post(`${BASE_URL}/api/dinggou-dingdans/${orderId}/redeem`, {}, {
      headers: { Authorization: `Bearer ${token}` }
    });
